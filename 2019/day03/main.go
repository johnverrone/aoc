...
func main() {
	...
	for i, line := range lines {
		...
		if i == 0 {
			wire1 = append(wire1, path...)
		}
		if i == 1 {
			wire2 = append(wire2, path...)
		}
	}

	// plot wire 1's coordinates
	visits := map[int][]Pos{0: {{y: 0, l: 0}}}
	movePoint(visits, wire1)

	dist := math.MaxInt
	x := 0
	y := 0
	l := 0
	for _, inst2 := range wire2 {
		c := mustInt(inst2[1:])
		dir := inst2[0]
		for range c {
			switch dir {
			...
			l++
			if yPoints, ok := visits[x]; ok {
				for _, y2 := range yPoints {
					if y == y2.y {
						// newDist := max(x, -x) + max(y, -y)
						newDist := y2.l + l
						if newDist < dist {
							dist = newDist
						}
...
func movePoint(v map[int][]Pos, wire []string) {
	x, y, l := 0, 0, 0
